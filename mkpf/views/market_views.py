...
def process(query_txt,size='',quantity=100):
    ...
    keyword = '%%{}%%'.format(query_txt.strip())
    shoes_id = Shoes.query.filter(Shoes.keyword.ilike(keyword)).first()

    if shoes_id :
        shoes_id = shoes_id.id
    ...
    for title, condition, size, price, seller, uploadtime, uri, img in fs_obj:
        ...
        total_list.insert(0, Marketprice(title=title, condition=condition, size=size, price=price,
                            ...
                            uri=uri, search_query=query_txt, img=img, shoesmodel_id=shoes_id))
        num +=1

    db.session.bulk_save_objects(total_list)
    db.session.commit()

    return ('Market 찾은값:{} DB에 저장한값:{}'.format(count,num))
